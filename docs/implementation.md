...
当前节点是 **Roadmap reset（只改文档，不写生产代码）**。在它关闭之前，只有 `AGENTS.md`、`README.md` 和 `docs/` 下五个 canonical 文档可以改动。
...
**研究问题**：显示名到 slug 的归一化如何既保证全局唯一、又保证用户能读出、输入并与保留路由区分？**反证问题**：哪种真实显示名（大小写、Unicode、同形字、保留字、超长）会让归一化产生用户无法理解或与系统路由冲突的 slug？

**权限 / 失败 / 直接证据**：只有成员看得到自己的 Space；并发同名创建由唯一索引决定唯一 winner，落败者看到建议而不是错误页；保留路径名被拒绝；无预置数据的新账号从登录走到 Space 页面。

**命令**：`go test ./internal/space/...`；`make test-db`；`pnpm --dir apps/web test`；`make check-product`。

**不做**：onboarding 表单、slug 改名、Space 删除、成员管理界面、Space 设置的其余部分。
...
| 门 | 问题 | 守护的节点 | 关闭它的证据 |
| --- | --- | --- | --- |
| G1 | slug 归一化、保留名与冲突建议规则 | 13 | 真实显示名样本 + 数据库唯一性行为 |
