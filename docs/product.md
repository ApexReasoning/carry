...
第一版明确组合 Pi 与 Codex adapter，并显示这台 Host 实际发现的默认 Pi/Codex Agent；尚未安装的 family 不产生占位 Agent，后来第一次发现时才创建，已经创建的 occurrence 后来从完整报告中缺失则保留身份并显示 Offline。紧随 MVP 的 P0 会增加至少 5–6 个具体 native Agent family，包括 DeepSeek Harness。当前界面仍没有 profile、slot、provider/model/runtime 目录或操作员 roster；新增 family 是受信任的产品代码与明确 Host 组合，不是用户下载或动态注册。Agent 清单上每个 Agent 显示：名字（在 Space 内唯一）、确定性生成的头像、所属 Host、人类 owner、状态（Active / Removed），以及在线、最近活跃和正在参与的 Work。名字由 PostgreSQL 按具体 family 独立分配：该 family 在 Space 的第一个 occurrence 使用 canonical base（如 `Pi`、`Codex`），之后为 `Pi 2`、`Codex 2` 等；不同 family 不为保持同一 Host 的数字配对而互相占位。Host 或人不提交名字，Removed 名字不复用。Space 内所有成员都可以选择其中的 Active Agent；第一版没有 per-Agent access mode。当具体 Agent 能可靠报告当前可选模型时，界面才显示模型选择；发现失败时使用 provider 默认值且不显示选择器。没有头像上传。

一个新 Agent 的人类 owner 是浏览器批准该逻辑 Host 的已认证成员，不来自 setup shell、进程发现内容或模型输出；该成员仍须是当前 Membership。每份在场报告必须包含每个明确组合 adapter 的完整发现结果：第一版的 Pi/Codex adapter 可以各发现 default occurrence，也可以在尚未安装时返回空结果；对已经发现并持久化的 occurrence，后续遗漏等同明确 absent。未来可增加 family 或同 family 的多个稳定 occurrence 而不改变公开报告结构。Occurrence key 是具体 adapter 从非模型的本机安装事实得到的稳定值，不是 persona、显示名或授权。明确 absent 立即显示离线，报告过期后按数据库时间显示离线，最近活跃取最近一次数据库确认 present 的时间。再次 setup 或发现只更新在场：不改写已有 Agent 的人类 owner、名字或生命周期，也不复活 Removed Agent。
