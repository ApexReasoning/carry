...
Current gate: **Issue #4 / Node 15 v4 research reset — connect one Host and automatically see its concrete native Agents as durable Space identities with owner, presence and lifecycle.** Issues #3/#5/#6/#7 and Node 14 are closed. The user rejected the prior closed Pi/Codex representation because post-MVP P0 adds at least 5–6 native Agent families, including DeepSeek Harness. V1 still proves Pi and Codex first, but Issue #4 must freeze a generic row/wire representation and explicit Host adapter composition before implementation. Node 15 ends at the visible durable Agent inventory; Node 16 owns Agent selection, the Agent-addressed Conversation, message execution and provider-session recovery.
...
- a Space has many logical Hosts; V1 explicitly composes Pi and Codex adapters, each able to discover one stable default occurrence; a not-installed family produces no Agent until discovered, while an omitted previously discovered occurrence becomes Offline. The durable identity key is the generic `(Machine, adapter key, adapter-local occurrence key)` so a later trusted in-repo adapter family or multiple stable occurrences do not require a schema or public-protocol redesign; there is no slot/profile/registry product, and every Agent belongs to exactly one Host;
- Agent is a durable fact owner and a user-visible identity: stable ID, Space, PostgreSQL-allocated Space-unique normalized name (the first occurrence of a recognized family uses its canonical base, later occurrences use an independently allocated numeric suffix), deterministic generated avatar, one human owner, one Host binding, immutable adapter/occurrence binding, Active/Removed lifecycle, created time; a new Agent's human owner is the still-active authenticated member who approved that Host connection, while repeat discovery never changes an existing Agent's owner, name or lifecycle; every Active Agent is selectable by members of its Space, and a Host going offline does not delete Agent identity;
- Machine owns a bounded complete collection of recognized adapter-occurrence present/absent observations; Online and Last active derive from Agent lifecycle plus database-time freshness, while current Runs and any optional model choice remain separate derived facts; copied Machine state is one logical Host, fresh setup after state loss is a new Host, and there is no physical-clone guess or dynamic provider/model/runtime registry;
...
Forbidden without a new user loss, identity, lifecycle and authority boundary: `Assignment`, `Coordinate`, a `Plan`/`Step`/`Artifact` owner, a generic `Effect`/`Action`/`Capability`, a Session owner, a participant owner, a notification owner, a scheduler owner, a workflow engine, a dynamic provider/model/runtime registry or product directory, and `Orchestrator`/`Manager`/`common`/`utils` layers. An Agent-owner immutable recognition table and a Host adapter set explicitly constructed at the binary composition root are not such a registry: they have no dynamic registration, discovery of arbitrary code, profiles, priorities, credentials, factories or fallback policy. Try an existing owner, field, function or local value first.
...
For the active Nodes 13–30 goal, the user has explicitly authorized uninterrupted progress across Node boundaries. After a Node's exact final-HEAD CI succeeds, update the mission state and begin the next Node instead of stopping for routine feedback. Escalate only an irreducible conflict that changes the journey, owner, authority, external consequence or approved scope and cannot be resolved from canonical sources, first-party research and bounded `carry.supervisor` advice.
