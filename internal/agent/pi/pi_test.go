...
printf '%s\n' \
  '{"id":"carry-prompt","type":"response","command":"prompt","success":true}' \
  '{"type":"message_end","message":{"role":"assistant","content":[{"type":"text","text":"{\"understanding\":\"Reference was consulted.\",\"next_step\":\"Continue the Work.\",\"review_required\":false}"}],"stopReason":"stop"}}' \
  '{"type":"agent_settled"}'
`)
	usePiFixture(t, binary)
...
`)
	...
	t.Run("failure response body is stopped", func(t *testing.T) {
		cancelled := make(chan struct{})
		server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
			response.WriteHeader(http.StatusServiceUnavailable)
			_, _ = response.Write([]byte("partial failure"))
...
`)
		if err != nil || output != "lookup_reference returned a failure status" {
			t.Fatalf("Pi failure-body output=%q error=%v", output, err)
		}
		select {
		case <-cancelled:
		case <-time.After(time.Second):
			t.Fatal("Pi kept consuming the failure response body")
		}
	})
...
func TestPiRejectsMalformedReferenceSettlement(t *testing.T) {
	for name, records := range map[string]string{
		...
		"final output before active call ends": strings.Join([]string{
			`{"id":"carry-prompt","type":"response","command":"prompt","success":true}`,
			`{"type":"tool_execution_start","toolCallId":"call-active","toolName":"lookup_reference"}`,
			`{"type":"message_end","message":{"role":"assistant","content":[{"type":"text","text":"{\"understanding\":\"Premature\",\"next_step\":\"Do not commit\"}"}],"stopReason":"stop"}}`,
			`{"type":"tool_execution_end","toolCallId":"call-active","toolName":"lookup_reference","isError":false}`,
			`{"type":"agent_settled"}`,
		}, "\n"),
