...
func (state *resultState) accept(record envelope) ([]byte, bool, error) {
	...
	case "message_end":
		...
		if message.Role == "assistant" {
			if message.StopReason == "stop" && len(state.activeReferenceCalls) != 0 {
				return nil, false, fmt.Errorf("%w: Pi produced final output before lookup_reference completed", host.ErrAgentOutcomeLost)
			}
