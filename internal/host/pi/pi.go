...
// Key returns Pi's stable Agent recognition key.
func (*Adapter) Key() agent.AdapterKey {
	return "pi"
}

// Observe omits an uninstalled Pi and retains a discovered unhealthy default occurrence.
...
func writePrompt(writer io.Writer, prompt string) error {
	if err := json.NewEncoder(writer).Encode(promptRequest{ID: promptRequestID,
		Type:    "prompt",
		Message: prompt}); err != nil {
